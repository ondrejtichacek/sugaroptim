...
def make_opt_plumed_file(frame,molecule_features,kwargs_dict):
    ...
    os.system("plumed driver --mf_xtc {0}_plumed_restraint_values.xtc --plumed {0}_plumed.dat".format(frame))

    restrain_features_values = np.loadtxt(frame+'_plumed_output_features.dat')
    restrain_features_values = restrain_features_values[1:]
    os.system('rm {0}_plumed.dat {0}_plumed_output_features.dat {0}_plumed_restraint_values.xtc'.format(frame))
...
def optimize_frame(i,molecule_features,kwargs_dict):
    ...
    # min
    os.system("gmx grompp -f md_min.mdp -c {0} -p {1} -o {0}_min.tpr -n index.ndx -po {0}_mdout1.mdp -maxwarn 5&> {0}_grompp_min.err".format(i,topol))
    os.system("mdrun_plumed -s {0}_min.tpr -v -deffnm {0}_min -nt 1 -plumed {0}_plumed_restraint.dat &> {0}_min.err".format(i))
    # MD opt
    os.system("gmx grompp -f md_opt.mdp -c {0}_min.gro -p {1} -o {0}_opt.tpr -n index.ndx -r {0}_min.gro -po {0}_mdout2.mdp -maxwarn 5 &> {0}_grompp_opt.err".format(i,topol))
    os.system("mdrun_plumed -s {0}_opt.tpr -v -deffnm {0}_opt -nsteps 20000 -nt 1 -plumed {0}_plumed_restraint.dat &> {0}_opt.err".format(i))
    # to xtc
...
def optimize_individual_frames(n_iteration,number_of_new_structures,molecule_features_class,**kwargs):
    ...
    os.system('cp -r {0}/needed_files/md_inp_files/* .'.format(p_dir))
    os.system("rm frame*gro.gro")
    ...
    os.system("gmx trjcat -f frame*opt.xtc -o frames_opt_cat.xtc -cat")
    molecule_features_class.write_plumed_print_features('plumed_features.dat','',1)

    os.system("plumed driver --plumed plumed_features.dat --mf_xtc frames_opt_cat.xtc")

...
class experiment:
    def __init__(self, experimental_data_av):
        ...
        if experimental_data_av[0] == 1:
            freq_ram, ram = np.loadtxt(
                exp_data_path+'/raman_icpu.dat', usecols=(0, 1), unpack=True)
            freq_roa, roa = np.loadtxt(
                exp_data_path+'/roa_icpu.dat', usecols=(0, 1), unpack=True)
        ...
        if experimental_data_av[2] == 1:
            atom_sch, sch = np.loadtxt(
                exp_data_path+'/sch.dat', usecols=(0, 1), unpack=True)
        else:
            atom_sch, sch = [[], []]

        # load nmr C data
        if experimental_data_av[3] == 1:
            atom_scc, scc = np.loadtxt(
                exp_data_path+'/scc.dat', usecols=(0, 1), unpack=True)
        else:
            atom_scc, scc = [[], []]

        # load nmr Jij data
        if experimental_data_av[4] == 1:
            jij, a1, a2 = np.loadtxt(
                exp_data_path+'/jij.dat', usecols=(0, 1, 2), unpack=True)
        else:
            jij, a1, a2 = [[], [], []]
...
class simulation:
    def __init__(self, path, molecule_features, experimental_data_av):
        ...
        def load_sim_roa_raman(path):
            ...
            # get the values
            freq, ram = np.loadtxt(
                path+'/raman_icpu.dat', usecols=(0, 1), unpack=True)
            freq, roa = np.loadtxt(path+'/roa_icpu.dat',
        ...
        for i in ind_folders:
            ...
            # scaling fnc
            try:
                self.scaling_function = np.loadtxt(
                    i+'/raman_scaling_function.dat')
            except:
                self.scaling_function = np.array([1, 1, 10, 1000])

            try:
                # freq/ram/roa
                freq_array[fwc], ram_array[fwc], roa_array[fwc] = load_sim_roa_raman(
                    i)
            ...
            # NMR data
            try:
                sch_array[fwc] = np.loadtxt(
                    i+'/sch.dat', usecols=1, unpack=True)
            except:
                pass
            try:
                scc_array[fwc] = np.loadtxt(
                    i+'/scc.dat', usecols=1, unpack=True)
            except:
                pass
            try:
                jij_array[fwc] = np.loadtxt(
                    i+'/jij.dat', usecols=0, unpack=True)
            except:
                pass

            # Load features
            fa_features[fwc] = list(np.loadtxt(
                i+'/plumed_output_features.dat', unpack=True)[1:])

            # Try to load additional plumed features
            try:
                fa_additional_features.append(
                    list(np.loadtxt(i+'/plumed_additional_output_features.dat', unpack=True)[1:]))
            except:
                pass
...
#             self.jij=np.array([])
        ...
        for i in self.exp.sch_idx:
            al_sch_sel.append(np.where(self.sch_idx == i)[0][0])
        self.sch = np.transpose(
            np.array([list(sch_array[:, i]) for i in al_sch_sel]))

        al_scc_sel = []
        for i in self.exp.scc_idx:
            al_scc_sel.append(np.where(self.scc_idx == i)[0][0])
        self.scc = np.transpose(
            np.array([list(scc_array[:, i]) for i in al_scc_sel]))
        ...
        self.jij = np.transpose(
            np.array([list(jij_array[:, i]) for i in sel_jij_idx]))
...
class data:
    def __init__(self, path, iteration, molecule_features, experimental_data_av, **kwargs):
        ...
        def filter_Jij_1(Jij):
            ...
            threshold = 1.0
            sa = (np.percentile(Jij, 90, axis=0) -
                  np.percentile(Jij, 10, axis=0)) > threshold
            return sa
        ...
        def filter_sch_1(sch_sim):
            ...
            threshold = 0.35
            sa = (np.percentile(sch_sim, 90, axis=0) -
                  np.percentile(sch_sim, 10, axis=0)) > threshold
            return sa
        ...
        def filter_scc_1(scc_sim):
            ...
            threshold = 3.0
            sa = (np.percentile(scc_sim, 90, axis=0) -
                  np.percentile(scc_sim, 10, axis=0)) > threshold
            return sa
        ...
        if ed_spinspin != 0:
            ...
            for idx, i in enumerate(zip(a, b)):
                if i[0] == i[1]:
                    ...
                    logger.warning(
                        "Warning, Jij shift does not match in sign (exp x sim)")
                    ...
                    logger.warning(
                        "Changing exp data sign to match simulation data.")
        ...
        if do_not_filter == False:
            ...
            if ed_spinspin != 0:
                ...
                if np.sum(Jij_sa) != len(Jij_sa):
                    logger.warning(
                        'Warning, following Jij values (pairs) have too small spread of sim values - omitting them in the optimization.')
                    print(
                        [i for i in zip(self.exp.jij_idx1[~Jij_sa], self.exp.jij_idx2[~Jij_sa])])
            ...
            if ed_shifts_H != 0:
                ...
                if np.sum(sch_sa_1) != len(sch_sa_1):
                    logger.warning(
                        'Warning, following H chemical shifts have too small spread of sim values - omitting them in the optimization.')
            ...
            if ed_shifts_C != 0:
                ...
                if np.sum(scc_sa_1) != len(scc_sa_1):
                    logger.warning(
                        'Warning, following C chemical shifts have too small spread of sim values - omitting them in the optimization.')
        ...
        Jij_atom_list = [i for i in zip(
            self.exp.nf_jij_idx1, self.exp.nf_jij_idx2)]

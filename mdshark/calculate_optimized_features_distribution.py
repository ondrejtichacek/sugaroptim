...
class calculate_new_distribution:
    def __init__(self,weights,sim_data,molecule_features,**kwargs):
        ...
        def calcg(x_my,x_array,weights_array):
            sigma=2*np.pi/25
            x=np.amin([np.abs(x_array-x_my),2*np.pi-np.abs(x_array-x_my)],axis=0)
        ...
        for idx,i in enumerate(np.arange(self.nfeatures)):
            if self.features_def_list_idx[idx]==1:
                x,y1,y2,y3,yfes,ygrad = calc_pop_fes_grad_dih(self.features[:,i],weights)
            elif self.features_def_list_idx[idx]==21:
                x,y1,y2,y3,yfes,ygrad = calc_pop_fes_grad_puck_phi(self.features[:,i],weights)
            elif self.features_def_list_idx[idx]==22:
                x,y1,y2,y3,yfes,ygrad = calc_pop_fes_grad_puck_theta(self.features[:,i],weights)
            elif self.features_def_list_idx[idx]==31:
                x,y1,y2,y3,yfes,ygrad = calc_pop_fes_grad_puck5_phs(self.features[:,i],weights)

...
def generate_restrained_MD_plumed_file_targeted_MD(molecule_features, n_iteration, kwargs_dict):
    with open("new_iteration_{0}/MD/plumed_restraint.dat".format(n_iteration), 'w') as fw:
        ...
        fw.write(
            molecule_features.write_plumed_stereochemical_centers_restraints_print(1000, ''))
        fw.write(
            molecule_features.write_plumed_improper_dihedrals_restraints_print(1000, ''))
        ...
        if molecule_features.features_def_list_idx[idx] == 1:
            ...
            with open("generate_MD_distributions/dist{0}/target{1}.dat".format(n_iteration, idx), 'w') as fw:
                fw.write("#! FIELDS {0} metad{1}.target der_{0}\n".format(
                    molecule_features.features_def_list[idx], idx))
                fw.write(
                    "#! SET min_{0} -pi\n".format(molecule_features.features_def_list[idx]))
                fw.write("#! SET max_{0} pi\n".format(
                    molecule_features.features_def_list[idx]))
                fw.write("#! SET nbins_{0}  {1}\n".format(
                    molecule_features.features_def_list[idx], len(x[:-1])))
                fw.write("#! SET periodic_{0} true\n".format(
                    molecule_features.features_def_list[idx]))
                for idx_x, xx in enumerate(x[:-1]):
                    fw.write("{0}  {1}  {2}\n".format(
                        x[idx_x], yfes[idx_x], ygrad[idx_x]))
...
def generate_new_structures(molecule_features, n_iteration, number_of_new_structures, final_distribution, **kwargs):
    ...
    logger.notice(
        f"Generating new structures with md simulation, it {n_iteration}")
    ...
    while cluster_sim_nt < ns:
        ...
        generate_restrained_MD_plumed_file_targeted_MD(
            molecule_features, n_iteration, kwargs)
        # generate target distribution files
        write_target_distribution(
            molecule_features, final_distribution, n_iteration)
        ...
        if glob.glob("new_iteration_{0}/MD/step*".format(n_iteration)) == []:
            run('cp new_iteration_{0}/MD/job{1}.xtc new_iteration_{0}/MD_trj/.'.format(
                n_iteration, cluster_sim_nt))
            run('cp new_iteration_{0}/MD/job{1}.gro new_iteration_{0}/MD_trj/structure_start_sim_prev.gro'.format(
                n_iteration, cluster_sim_nt))
            cluster_sim_nt += 1
            pbar.update(1)
        else:

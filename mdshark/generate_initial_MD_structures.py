...
def generate_new_structures(molecule_features, n_iteration, number_of_new_structures, **kwargs):
    ...
    while cluster_sim_nt < number_of_new_structures:
        ...
        molecule_features.write_plumed_file_MD0(
            "new_iteration_{0}/MD/plumed_restraint.dat".format(n_iteration), '', 1000, kwargs)
    ...
    molecule_features.write_plumed_file_MD0_check_features(
        "new_iteration_{0}/MD_trj/plumed_features.dat".format(n_iteration), '', 1)
    c_path = os.getcwd()
    os.chdir('new_iteration_{0}/MD_trj'.format(n_iteration))

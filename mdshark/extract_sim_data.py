...
def average_methyl_amonia_groups(trjdir):
    ...
    mol_name_0 = [t.topology.atom(
        i).element.symbol for i in np.arange(len(mol_xyz))]
...
def get_nmr_spinspin_data(wdir_a, molecule_features):
    ...
    import itertools
    atom_combinations = list(
        itertools.combinations_with_replacement(atom_number_list, 2))
    ...
    for jf in next(os.walk(wdir_a))[1]:
        ...
        for i in reversed(linestp):
            jj_lines.pop(i)
        arr = [[float((i.replace('D', 'E'))) for i in line]
            ...
            for eq_group in equivalent:
                if i not in eq_group:
                    ...
                    for idx_eq, j in enumerate(eq_group):
                        pair = sorted([i, j])
                        # print(pair)
                        idx = np.where((Jij_curr[:, 1] == pair[0]) & (
                            Jij_curr[:, 2] == pair[1]))[0][0]
        ...
        with open(str(wdir_a)+str(jf)+'/jij.dat', 'w') as f_jij, open(str(wdir_a)+str(jf)+'/jij_only.dat', 'w') as f_jij_only:
            ...
            for i in range(len(Jij_curr)):
                f_jij.write('{0:5f} {1:10d} {2:10d} \n'.format(
                    Jij_curr[i][0], int(Jij_curr[i][1]), int(Jij_curr[i][2])))
            for i in range(len(Jij_curr)):
    ...
    Jij_av = np.hstack(
        (np.array([[i] for i in Jij_av_values/c]), Jij_av_atoms))
    ...
    with open(str(wdir_a)+'jij.dat', 'w') as f_jij,\
        ...
        for i in range(len(Jij_av)):
            f_jij.write('{0:5f} {1:10d} {2:10d} \n'.format(
                Jij_av[i][0], int(Jij_av[i][1]), int(Jij_av[i][2])))
        for i in range(len(Jij_av)):
            f_jij_only.write('{0:5f}\n'.format(Jij_av[i][0]))
...
def get_vib_data(wdir_a, kwargs_dict):
    ...
    for j in next(os.walk(wdir_a))[1]:
        ...
        try:
            ...
            logger.warning(
                "Use raman shifting function is not defined/ill defined, setting to True")
        ...
        if delete_file == 1:
            logger.warning(
                'Wrong (****** problem) vib intensity, deleting {0}'.format(wdir_a+j))
        ...
        with open(wdir_a+j+'/g_final_ROA_calc.dat', 'w') as f_w:
            for i in range(0, len(fun_freq_a)):
                f_w.write('{0:20s} {1:20s} {2:20s}  \n'.format(
                    str(fun_freq_a[i]), str(fun_raman_icpu_a[i]), str(fun_roa_icpu_a[i])))
        ...
        sf_array = (par_a*1/(1+np.exp((-fun_freq_a+par_d)/par_c)) +
                    par_b*(1-1/(1+np.exp((-fun_freq_a+par_d)/par_c))))
        ...
        for k, freq_v in enumerate(range(minv, maxv+1)):
            ...
            suma_raman1 = normalization_factor*suma_raman / \
                freq_v*(1/(1-math.exp(q3*freq_v)))
            raman_spectra[k] = suma_raman1

            suma_roa = np.sum(y1_roa_a/((freq_v-y1_freq)**2+q2))
            suma_roa1 = normalization_factor*suma_roa / \
                freq_v*(1/(1-math.exp(q3*freq_v)))
        ...
        with open(str(wdir_a)+str(j)+'/raman_icpu.dat', 'w') as f_raman,\
            ...
            for i in range(0, len(freq_spectra)):
                f_raman.write('{0:10s} {1:10s} \n'.format(
                    str(freq_spectra[i]), str(raman_spectra[i])))
                f_roa.write('{0:10s} {1:10s} \n'.format(
                    str(freq_spectra[i]), str(roa_spectra[i])))
    ...
    with open(wdir_a+'raman_icpu.dat', 'w') as f_raman,\
        ...
        for i in range(0, len(freq_spectra_av)):
            f_raman.write('{0:10s} {1:10s} \n'.format(
                str(freq_spectra_av[i]), str(raman_spectra_av[i])))
            f_roa.write('{0:10s} {1:10s} \n'.format(
                str(freq_spectra_av[i]), str(roa_spectra_av[i])))
...
def calculate_all_sim_data(path, molecule_features, **kwargs):
    ...
    for j in glob.glob(folders):
        ...
        for ipath in input_files:
            ...
            try:
                calculate_which_spectra = what_to_calculate(
                    ipath+'/calculate.dat')
                # Check that all have been calculated, if not delete
                calculated_check = all_calculated(
                    calculate_which_spectra, ipath)
                if calculated_check == 0:
                    continue
                # Check that the calculations have normal terminations
                calculated_check = normal_termination(
                    calculate_which_spectra, ipath)
                if calculated_check == 0:
                    continue
            except FileNotFoundError:
                logger.warning(
                    f'Have not found calculate.dat file, deleting {ipath}')
                shutil.rmtree(ipath)
        ...
        if calculate_which_spectra[1] == 1:
            ...
            try:
                intercept_H, slope_H = np.loadtxt(
                    ipath+'/nmr_h_shifts_regression_par.dat')
                intercept_C, slope_C = np.loadtxt(
                    ipath+'/nmr_c_shifts_regression_par.dat')
            except:
                logger.critical(
                    "NMR chemical shifts regression parameters have not been correctly defined!")
                logger.critical(
                    "Using default parameters - this is not correct!")

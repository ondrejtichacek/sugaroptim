...
def make_opt_plumed_file(frame, molecule_features, kwargs_dict):
    # Make plumed list
    molecule_features.write_plumed_print_features(
    ...
    molecule_features.write_plumed_file_MD_opt(
        frame+'_plumed_restraint.dat', frame+'_', 1000, kwargs_dict, restrain_features_values)
...
def optimize_individual_frames(n_iteration, number_of_new_structures, molecule_features_class, **kwargs):
    ...
    xtc_file = p_dir + \
        "/new_iteration_{0}/MD_trj/job_cat.xtc".format(n_iteration)
    ...
    molecule_features_class.write_plumed_print_features(
        'plumed_features.dat', '', 1)

...
def recalculate_shifting_function(sim_data, original_sim_data, exp_data, overall_weights_min, kwargs_dict):
    ...
    if recalculate_raman_shifting_function == True:
        try:
            set_raman_shifting_function_parameters_range = kwargs_dict[
                'set_raman_shifting_function_parameters_range']
        except:
            logger.warning(
                "Problem with setting raman shifting function parameters range. Falling back to default values.")
            set_raman_shifting_function_parameters_range = [
                (0.95, 1.05), (0.95, 1.05), (15, 1000), (500, 2500)]
    ...
    for idx, i in enumerate(sim_data[1]):
        raman_data_scaled[idx], roa_data_scaled[idx] = shift_raman_roa_data(
            sf_raman_params, exp_data[0], sim_data[1][idx], sim_data[2][idx], exp_data[1], exp_data[2])
...
def par_optimization(params, sim_data, exp_data, experimental_data_av, weights_indices_kept, original_weights, original_sim_data, kwargs_dict):
    ...
    if sim_data[3].size != 0:
        ...
        sch_diff = c_sch_0*c_sch * \
            np.sum((np.abs(sim_sch-exp_data[3]))**c_sch_exp)/len(sim_sch)
    else:
        sch_diff = 0
    if ed_shifts_H == 1:
        difference += sch_diff
    ...
    if sim_data[4].size != 0:
        ...
        scc_diff = c_scc_0*c_scc * \
            np.sum((np.abs(sim_scc-exp_data[4]))**c_scc_exp)/len(sim_scc)
    else:
        scc_diff = 0
    if ed_shifts_C == 1:
        difference += scc_diff
    ...
    if sim_data[5].size != 0:
        ...
        jij_diff = c_jij_0*c_jij * \
            np.sum((np.abs(sim_jij-exp_data[5]))**c_jij_exp)/len(sim_jij)
    else:
        jij_diff
    if ed_spinspin == 1:
        difference += jij_diff
    ...
    if bias_n_structures_reached == True:
        if structures_selected_f > bias_n_structures_reached_f:
            ...
            str_bias = error_sum * \
                (bias_n_structures_reached_f - structures_selected/len(weights_i))
    ...
    if (ed_raman == 1) and (recalculate_raman_shifting_function == True):
        if counter in [1_000, 10_000, 25_000, 50_000, 100_000, 175_000, 250_000, 400_000] + [750_000 + 250_000*i for i in range(0, 100)]:
            ...
            logger.info(
                "step:{0}, recalculating Raman/ROA shifting function.".format(counter))
            ...
            opt_result = optimize.differential_evolution(scaling_function_par_optimization,
                                                         bounds=set_raman_shifting_function_parameters_range,
                                                         args=(
                                                             freq, ram, roa, exp_data[1], exp_data[2]),
                                                         maxiter=100)
    ...
    if counter % 1 == 0:
        error_data_array_i.append(
            [ram_diff, roa_diff, sch_diff, scc_diff, jij_diff, structures_selected])
   ...
   # weights/error min
    ...
    try:
        if len(kwargs['stop_optimization_when_n_structures_reached']) == 1:
            stop_optimization_when_n_structures_reached = kwargs[
                'stop_optimization_when_n_structures_reached']
        else:
            if all_data.iteration >= 1:
                stop_optimization_when_n_structures_reached = kwargs[
                    'stop_optimization_when_n_structures_reached'][all_data.iteration-1]
            else:
    ...
    try:
        stop_optimization_when_error_not_decreasing = kwargs[
            'stop_optimization_when_error_not_decreasing']
    except:
    ...
    try:
        ...
        logger.info(
            f"Relative optimization weights are: {kwargs['relative_optimization_weights']}")
    except:
        logger.warning(
            "Problem with setting relative optimization weights - uset-input. Setting to defaults.")
    ...
    logger.notice("Optimization will stop when # structures is less than {}.".format(
        stop_optimization_when_n_structures_reached))
    logger.notice(
    ...
    opt_result = optimize.differential_evolution(par_optimization,
                                                 bounds=b0,
                                                 tol=10**-10,
                                                 args=(sim_data, exp_data, experimental_data_av,
                                                       weights_indices_kept, original_weights, sim_data, kwargs),
                                                 popsize=set_pop,
                                                 maxiter=set_maxiter,
    ...
    if (recalculate_raman_shifting_function == True) and (experimental_data_av[0] == 1):
        recalculate_shifting_function(
            sim_data, sim_data, exp_data, overall_weights_min, kwargs)
    ...
    while (nonzero_structures > stop_optimization_when_n_structures_reached):
        ...
        opt_result = optimize.differential_evolution(par_optimization,
                                                     bounds=b0,
                                                     tol=10**-10,
                                                     args=(sim_data_truncated, exp_data, experimental_data_av,
                                                           weights_indices_kept, original_weights, sim_data, kwargs),
                                                     popsize=set_pop,
                                                     maxiter=set_maxiter,)

        logger.verbose("Best results so far, it: ", error_min_counter, ',sum_error: ', round(sum(
            error_data_array_i[error_min_counter][:-1]), 2), ',error array: ', np.round(error_data_array_i[error_min_counter], 2))
        ...
        if (recalculate_raman_shifting_function == True) and (experimental_data_av[0] == 1):
            recalculate_shifting_function(
                sim_data_truncated, sim_data, exp_data, overall_weights_min, kwargs)
        ...
        if nonzero_structures_after_opt < stop_optimization_when_n_structures_reached:
            logger.notice("Obtained less than {0} structures. Stopping the optimization.".format(
                stop_optimization_when_n_structures_reached))
            nonzero_structures = np.sum(weights > 0.0001*np.max(weights))
            break
...
#         par_optimization(overall_weights_min,sim_data,exp_data,experimental_data_av,np.arange(np.shape(sim_data[0])[0]),overall_weights_min,sim_data,kwargs)
    ...
    stored_results = store_results(
        overall_weights_min, error_min, error_data_array_i, error_min_counter)

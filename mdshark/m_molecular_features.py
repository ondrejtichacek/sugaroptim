...
Moreover it will write the initial plumed.dat file.
    ...
    def __init__(self, name, itp_file, **kwargs):
        ...
        if 'select_central_mol' in kwargs:
            ...
            logger.verbose(
                f"Central mol selection: {kwargs['select_central_mol']}")
        ...
        # get elements
        elements = np.array([i.element.symbol for i in t.topology.atoms])[
            molecule_idx]
        ...
        ub2 = np.array([sorted(list(i)) for i in list(
            dict.fromkeys({tuple(row) for row in ub2}))])+1
        ub3 = np.array([sorted(list(i)) for i in list(
            dict.fromkeys({tuple(row) for row in bonds13}))])+1
        ...
        dih_set = np.array([list(i) for i in list(
            dict.fromkeys({tuple(row) for row in dih_list}))])
        ...
        new_dih_setX = np.array([list(i) for i in list(
            dict.fromkeys({tuple(row) for row in new_dih_set}))])
        ...
        final_dihedral_a = np.array(
            final_dih_set+np.ones(np.shape(final_dih_set)), dtype=int)
        self.dihedrals = final_dihedral_a
        self.all_dihedrals = np.array(
            np.array(dih_list) + np.ones(np.shape(dih_list)), dtype=int)
        ...
        for i in self.all_dihedrals:
            if elements[i[0]-1] == 'H' and elements[i[1]-1] == 'N' and\
                ...
                amide_bond.append(i)
        # now filter them
        amide_bond = np.array([list(x)
                              for x in set(tuple(x) for x in amide_bond)])
        ...
        pg_all = list(self.pg_amide_bond)+list(self.pg_nitrogen) + \
            list(self.pg_sulphur)+list(self.pg_carboxylate)
                            ...
                            for l4 in list4:
                                ...
                                for lx4a in lx4:
                                    ...
                                    for l5 in list5:
                                        ...
                                        for lx5a in lx5:
                                            list6 = (
                                                np.where(bonds == lx5a))[0]
                                            for l6 in list6:
                                                lx6 = [
                                                    bonds[l6][bonds[l6] != lx5a][0]]
                                                for lx6a in lx6:
                                                    list7 = (
                                                        np.where(bonds == lx6a))[0]
                                                    for l7 in list7:
                                                        lx7 = [
                                                            bonds[l7][bonds[l7] != lx6a][0]]
                                                        dih = [
                                                            i, lx2[0], lx3[0], lx4[0], lx5[0], lx6[0], lx7[0]]
                                                        if len(f5(dih)) == 6:
                                                            dih_list.append(
                                                                dih)
        ...
        if possible_dih != []:
            ...
            slist = np.array(
                np.unique(np.concatenate(od_all, axis=0)), dtype=int)
            sarray = []
            ...
            puck_final_list = np.array(
                puck_final_list+np.ones(np.shape(puck_final_list)), dtype=int)
    ...
    # Lastly check if there is a plumed_redefined.dat file, if yes, then use it
        try:
            ...
            print(
                "Found plumed_redefined.dat file. Using these definition for features instead.")
        except:
            pass
    ...
    def write_plumed_stereochemical_centers_restraints(self):
        ...
        if self.stereochemical_centers != []:
            ...
            str_to_write += '{0}'.format(
                self.stereochemical_centers[len(self.stereochemical_centers)-1][1])
            str_to_write += ' KAPPA='
            for idx, i in enumerate(self.stereochemical_centers[:-1]):
    ...
    def write_plumed_stereochemical_centers_restraints_print(self, write_frequency, plumed_output_folder):
        ...
        if self.stereochemical_centers != []:
            ...
            str_to_write += 'stereochemical_restraint.bias STRIDE={0} FILE={1}plumed_output_stereochemical_centers_bias.dat\n\n'.format(
                write_frequency, plumed_output_folder)
        return str_to_write
    ...
    def write_plumed_improper_dihedrals_restraints(self):
        ...
        if self.unique_improper_dihedrals != []:
            ...
            str_to_write += 'improper_dih{0}'.format(
                len(self.unique_improper_dihedrals)-1)
            str_to_write += ' AT='
            for idx, i in enumerate(self.unique_improper_dihedrals[:-1]):
                str_to_write += '{0},'.format(i[1])
            str_to_write += '{0}'.format(
                self.unique_improper_dihedrals[len(self.unique_improper_dihedrals)-1][1])
            str_to_write += ' KAPPA='
            for idx, i in enumerate(self.unique_improper_dihedrals[:-1]):
    ...
    def write_print_improper_dihedrals(self, write_frequency, plumed_output_folder):
        ...
        if self.unique_improper_dihedrals != []:
            ...
            str_to_write += "improper_dih{0}".format(
                len(self.unique_improper_dihedrals))
    ...
    def write_print_amide_bond(self, write_frequency, plumed_output_folder):
        ...
        if self.amide_bond != []:
            ...
            str_to_write += "amide_bond{0}".format(
                len(self.unique_improper_dihedrals))
    ...
    def write_plumed_print_features(self, plumed_file_name, plumed_output_folder, write_frequency):
        with open(plumed_file_name, 'w') as fw:
            fw.write(self.write_plumed_features())
            fw.write(self.write_plumed_features_print(
                write_frequency, plumed_output_folder))

    def write_additional_plumed_print_features(self, plumed_file_name, plumed_output_folder):
        run('cp needed_files/plumed_additional_features.dat {0}'.format(
            plumed_file_name))
        run("sed -i 's|FILEDEST|{0}|g' {1} ".format(
            plumed_output_folder, plumed_file_name))
    ...
    def write_plumed_file_MD0(self, plumed_file_name, plumed_output_folder, write_frequency, kwargs_dict):
        with open(plumed_file_name, 'w') as fw:
            ...
            fw.write(self.write_plumed_stereochemical_centers_restraints_print(
                write_frequency, plumed_output_folder))
            fw.write(self.write_plumed_improper_dihedrals_restraints_print(
                write_frequency, plumed_output_folder))
            fw.write(self.write_plumed_amide_bond_restraints_print(
                write_frequency, plumed_output_folder))
            fw.write(self.write_plumed_features_print(
                write_frequency, plumed_output_folder))
    ...
    def write_plumed_file_MD_opt(self, plumed_file_name, plumed_output_folder, write_frequency, kwargs_dict, restrain_features_values):
        with open(plumed_file_name, 'w') as fw:
            ...
            fw.write(self.write_plumed_improper_dihedrals_restraints())
            fw.write(self.write_plumed_amide_bond_restraints(kwargs_dict))
            fw.write(self.write_plumed_features_restraints(
                restrain_features_values))
